...
def remove_horizontal_lines(mask):
    ...
    if lines is not None:
        ...
        for line in lines:
            ...
            if horizontal_angle <= HORIZONTAL_ANGLE_MAX:
                cv2.line(result, (x1, y1), (x2, y2), 0, LINE_REMOVE_THICKNESS)
                removed += 1
    ...
    for i in range(1, num_labels):
        ...
        # 追加: 縦方向の広がりが小さい成分も横線とみなして除去
        # (縦のレーザー線は基本的に画像の高さ方向にある程度伸びるはず)
        if height < mask.shape[0] * 0.03:  # 画像高さの3%未満なら除去
            continue
